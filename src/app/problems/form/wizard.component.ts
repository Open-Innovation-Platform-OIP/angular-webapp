...
import { Component, ElementRef, ViewChild, OnInit, OnChanges, OnDestroy, AfterViewInit, SimpleChanges } from '@angular/core';
...
export class WizardComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
    ...
    private _filter(value: string): string[] {
        ...
        return Object.keys(this.tagService.allTags).filter(sector => sector.toLowerCase().indexOf(filterValue) === 0);
    }
  ...
  displayFieldCss(form: FormGroup, field: string) {
                ...
                this.apollo
                    ...
                    .valueChanges.subscribe(result => {
                        if (result.data.problems.length >= 1 && result.data.problems[0].id) {
                            ...
                            this.sectors = result.data.problems[0].problem_tags.map(tagArray => {
        ...
        $('.card-wizard').bootstrapWizard({
            ...
            onNext: function(tab, navigation, index) {
                ...
                if (!$valid) {
                    ...
                    if (confirm('Are you sure you want to add a new problem and not enrich an existing one?')) {
                        canProceed = true;
                        return true;
                    } else {
                        return false;
    ...
    removePhoto(index) {
        this.filesService
            .deleteFile(
                this.problem['image_urls'][index]['key']
            )
            .promise()
            .then(data => {
    ...
    removeVideo(index: number) {
        this.filesService
            .deleteFile(
                this.problem['video_urls'][index]['key']
            )
            .promise()
            .then(data => {
    ...
    isComplete() {
        return this.problem.title && this.problem.description && this.problem.organization && this.problem.min_population && this.problem.location;
    }
    ...
    submitProblemToDB() {
        const upsert_problem = gql`
            mutation upsert_problem($problems:[problems_insert_input!]!) {
                insert_problems(objects: $problems, on_conflict: {constraint: problems_pkey, update_columns: [title, description, location, resources_needed, organization, impact, extent, beneficiary_attributes, image_urls, video_urls, owners, min_population, modified_at, featured_url, featured_type, embed_urls]}) {
                affected_rows
                returning {
                    id
                }
            ...
            if (result.data.insert_problems.returning.length > 0) {
                ...
                if (problem_tags.size > 0) {
                    const upsert_problem_tags = gql`
                        mutation upsert_problem_tags($problems_tags:[problems_tags_insert_input!]!) {
                            insert_problems_tags(objects: $problems_tags, on_conflict: {constraint: problems_tags_pkey, update_columns: [tag_id, problem_id]}) {
                            affected_rows
                            returning {
                                tag_id
                                problem_id
